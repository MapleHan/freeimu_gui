...
from freeimu_cal import Ui_FreeIMUCal
...
class FreeIMUCal(QMainWindow, Ui_FreeIMUCal):
    ...
    def __init__(self):
        ...
        self.serialPortEdit.setText(self.settings.value("calgui/serialPortEdit", "COM1"))
        ...
        self.acc3D_sp = gl.GLScatterPlotItem(pos=numpy.array([0, 0, 0]).transpose())
        self.acc3D.addItem(self.acc3D_sp)

        self.magn3D_sp = gl.GLScatterPlotItem(pos=numpy.array([0, 0, 0]).transpose())
    ...
    def serial_connect(self):
        ...
        try:
            ...
            if self.ser.isOpen():
                ...
                time.sleep(3)
                self.ser.flushInput() # clear serial interface buffers
                self.ser.flushOutput()
                time.sleep(0.1)
                self.ser.write(b'v')  # ask version
                self.set_status(
                    "Connected to: " + str(self.ser.readline()))  # todo hangs if a wrong serial protocol has been loaded. To be fixed.
    ...
    def sampling_start(self):
        ...
        self.serWorker.new_data_signal.connect(self.newData)
    ...
    def calibrate(self):
        ...
        mp = gl.GLScatterPlotItem(pos=magn3D_cal_data, color=(1, 1, 1, 1), size=2)
        self.magn3D_cal.addItem(mp)
...
"""
        calibration_h_text = text % (
        self.acc_offset[0], self.acc_offset[1], self.acc_offset[2], self.acc_scale[0], self.acc_scale[1],
        self.acc_scale[2], self.magn_offset[0], self.magn_offset[1], self.magn_offset[2], self.magn_scale[0],
        self.magn_scale[1], self.magn_scale[2])

        calibration_h_folder = QFileDialog.getExistingDirectory(self,
                                                                "Select the Folder to which save the calibration.h file")
        ...
        self.set_status("Calibration saved to: " + str(
            calibration_h_folder) + calibration_h_file_name + " .\nRecompile and upload the program using the FreeIMU library to your microcontroller.")

    def save_calibration_eeprom(self):
        self.ser.write(b'c')
        # pack data into a string
        offsets = pack('<hhhhhh', self.acc_offset[0], self.acc_offset[1], self.acc_offset[2], self.magn_offset[0],
                       self.magn_offset[1], self.magn_offset[2])
        scales = pack('<ffffff', self.acc_scale[0], self.acc_scale[1], self.acc_scale[2], self.magn_scale[0],
                      self.magn_scale[1], self.magn_scale[2])
...
class SerialWorker(QThread):
    # define new signals - need to be done at Class level
    new_data_signal = pyqtSignal('PyQt_PyObject') #  pass a Python list object
    ...
    def run(self):
        ...
        while not self.exiting:
            # determine word size
            self.ser.write(b'b')
            self.ser.write(str.encode(chr(count)))
            ...
            self.new_data_signal.emit(reading)
            print(".", end=' ')
...
if __name__ == "__main__":
    sys.excepthook = except_hook  # traceback handling
